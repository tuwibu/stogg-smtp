...
```bash
cp ..codex/hooks/notifications/.env.example ~/.codex/.env
```
...
Add to your `.codex/hooks.json` or `.claude/settings.local.json`:
...
- `Notification` - Codex notification events (input prompts, etc.)
...
```bash
echo '{"hook_event_name":"Stop","cwd":"'$(pwd)'","session_id":"test123"}' | node ..codex/hooks/notifications/notify.cjs
```

Expected output (if Telegram and Discord configured):
```
[env-loader] Loaded: /home/user/.codex/.env
...
Cross-platform hook blocking heavy directories to improve Codex performance.
...
**Testing:**
```bash
echo '{"tool_input":{"command":"ls node_modules"}}' | node ..codex/hooks/scout-block.cjs
```

## Workflow Artifact Gate

Validates structured review artifacts generated by `fix` and `cook`.
Manual use:

```bash
node ..codex/hooks/workflow-artifact-gate.cjs --stage finalize --artifact-dir plans/example/reports/harness
```

Opt in through `.codex/.ck.json`:
...
Then add hook commands to `.codex/hooks.json`:
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node \"..codex/hooks/workflow-artifact-gate.cjs\""
          }
        ]
      }
    ],
...
- Verify hooks are added to `.codex/hooks.json`
- Check Codex is using correct settings file
