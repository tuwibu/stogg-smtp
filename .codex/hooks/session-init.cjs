...
function cleanupOrphanedShadowedSkills() {
  const shadowedDir = path.join(process.cwd(), '.agents', 'skills', '.shadowed');
...
async function main() {
  ...
  try {
    ...
    // Resolve once, reused for session state and staticEnv below.
    ...
    if (sessionId) {
      updateSessionState(sessionId, prev => ({
        ...
        // Persist branch in session state so consumers work on Codex, which has no
        // env-file mechanism (CK_GIT_BRANCH is never injected there).
        gitBranch: gitBranch || '',
        timestamp: Date.now(),
        source,
    ...
    // Extract task list ID for Codex task coordination (shared helper)
    ...
    if (envFile) {
      ...
      // Codex task coordination - enables multi-session/subagent coordination
      ...
      writeEnv(envFile, 'CK_AGENT_SETTINGS_DIR', staticEnv.codexSettingsDir);
    ...
    if (teamInfo) {
      ...
      console.log(`    Team config: Codex subagent metadata`);
      console.log(`    Use Codex subagent workflows for orchestration.`);
    }
    ...
    // MITIGATION: Issue #277 - Auto-compact can bypass user approval approval gates
    // When context is compacted mid-workflow, the summarization may lose "pending approval" state.
    // This warning reminds the agent to verify if user approval was pending before proceeding.
    // The runtime should preserve pending interactive state during compaction.
    if (source === 'compact') {
      console.log(`\n⚠️ CONTEXT COMPACTED - APPROVAL STATE CHECK:`);
      console.log(`If you were waiting for user approval via user approval (e.g., Step 4 review gate),`);
