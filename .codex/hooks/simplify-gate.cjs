...
function loadConfig(cwd) {
  ...
  let user = {};
  for (const rel of [path.join('.codex', '.ck.json'), path.join('.claude', '.ck.json'), '.ck.json']) {
    try {
...
function formatMessage(signals, breaches, severity) {
  ...
  return [
    ...
    `  Spawn the code-simplifier subagent with: "Simplify keeping behavior identical: <files>"`,
