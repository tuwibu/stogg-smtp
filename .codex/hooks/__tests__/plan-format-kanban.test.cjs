 ...
 * Run: node ..codex/hooks/__tests__/plan-format-kanban.test.cjs
 *
 * Tests:
...
/**
 * Build JSON payload simulating Codex tool call
 */
...
describe('plan-format-kanban.cjs', () => {
  ...
  describe('Direct status edit detection (Edit tool)', () => {
    ...
    it('Edit tool with status value in table row → warns to use CLI', async () => {
      ...
      assert.ok(
        result.output.additionalContext.includes('claudekit plan'),
        `Should mention claudekit plan CLI, got: ${result.output.additionalContext}`
      );
    });
    ...
    it('Edit tool with In Progress in table row → warns to use CLI', async () => {
      ...
      assert.strictEqual(result.output.continue, true);
      assert.ok(result.output.additionalContext, 'Should warn about direct status edit');
      assert.ok(result.output.additionalContext.includes('claudekit plan'));
    });
...
`;
  ...
  describe('CLI command syntax in warnings', () => {
    ...
    it('warning includes claudekit plan check <id> exact syntax', async () => {
      ...
      assert.ok(ctx.includes('claudekit plan check'), 'Warning must mention claudekit plan check');
      assert.ok(ctx.includes('--start'), 'Warning must include --start flag for in-progress');
      assert.ok(ctx.includes('claudekit plan uncheck'), 'Warning must mention claudekit plan uncheck');
    });
