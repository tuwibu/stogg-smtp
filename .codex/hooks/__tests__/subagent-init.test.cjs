...
describe('subagent-init.cjs', () => {
  ...
  describe('Issue #540: ck plan CLI injection for plan-aware agents', () => {
    ...
    it('injects claudekit plan CLI section for agents that may update plan state', async () => {
      const planAwareTypes = ['planner', 'project-manager', 'code-simplifier', 'developer'];
      ...
      for (const agentType of planAwareTypes) {
        ...
        assert.ok(
          context.includes('claudekit plan check'),
          `Agent type '${agentType}' should receive 'claudekit plan check' command`
        );
        assert.ok(
          context.includes('claudekit plan check') && context.includes('--start'),
          `Agent type '${agentType}' should receive 'claudekit plan check <id> --start' for in-progress`
        );
        assert.ok(
          context.includes('claudekit plan uncheck'),
          `Agent type '${agentType}' should receive 'claudekit plan uncheck' command`
        );
        assert.ok(
    ...
    it('does NOT inject claudekit plan CLI for non-plan agents', async () => {
    ...
    it('injects --dir flag in plan CLI when activePlan is provided', async () => {
      const activePlanPath = '/absolute/path/to/plan/260602-1000-test-plan';

      const result = await runHook({
        agent_type: 'planner',
        agent_id: 'test-540-dir',
        cwd: process.cwd(),
        session_id: 'test-session-with-plan'
      });

      const context = result.output?.hookSpecificOutput?.additionalContext || '';

      // When activePlan is set (via session), the CLI commands should include --dir
      // The hook resolves activePlan from session state and passes it to buildPlanCliSection()
      // Note: In test environment without actual session files, activePlan may be empty
      // Real test happens in smoke test with actual plan directory

      // Still verify the structure is there and --dir parameter is available if plan set
      assert.ok(
        context.includes('claudekit plan check'),
        'Should include plan check command'
      );
      assert.ok(
        context.includes('--start'),
        'Should include --start flag for in-progress'
      );
    });
  ...
  describe('Issue #291: CWD and Git Root Handling', () => {
    ...
    it('resolves different git root when payload.cwd is in subdirectory', async () => {
      ...
      // Use ..codex/hooks as subdirectory
