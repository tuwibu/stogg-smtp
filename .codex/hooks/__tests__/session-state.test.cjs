 ...
 * Run: node --test ..codex/hooks/__tests__/session-state.test.cjs
 */
...
describe('session-state.cjs', () => {
  ...
  it('allows a valid empty the current task checklist snapshot to clear cached todos', async () => {
    ...
    fs.writeFileSync(transcriptPath, JSON.stringify({
      ...
      message: {
        content: [{
          type: 'tool_use',
          id: 'todo-write-1',
          name: 'the current task checklist',
          input: { todos: [] }
        }]
      }
    }) + '\n');
    ...
    const result = await runHook({
      hook_event_name: 'PostToolUse',
      tool_name: 'the current task checklist',
      session_id: sessionId,
      cwd: process.cwd(),
      transcript_path: transcriptPath
    });
    ...
    assert.strictEqual(result.exitCode, 0, 'Hook should exit with code 0');
    const sessionState = JSON.parse(fs.readFileSync(sessionPath, 'utf8'));
    assert.strictEqual(sessionState.statusline.todos.length, 0, 'A valid empty the current task checklist should clear the cached todo list');
  });
});

describe('extractSessionData branch/plan sourcing (Codex parity)', () => {
  const { extractSessionData } = require('../lib/session-state-manager.cjs');
  const SAVED_ENV = { branch: process.env.CK_GIT_BRANCH, plan: process.env.CK_ACTIVE_PLAN };

  afterEach(() => {
    if (SAVED_ENV.branch === undefined) delete process.env.CK_GIT_BRANCH;
    else process.env.CK_GIT_BRANCH = SAVED_ENV.branch;
    if (SAVED_ENV.plan === undefined) delete process.env.CK_ACTIVE_PLAN;
    else process.env.CK_ACTIVE_PLAN = SAVED_ENV.plan;
  });

  it('reads branch/plan from session state (Codex: no env-file, CK_* absent)', () => {
    const sessionId = `extract-state-test-${Date.now()}`;
    track(path.join(os.tmpdir(), `ck-session-${sessionId}.json`));
    fs.writeFileSync(
      path.join(os.tmpdir(), `ck-session-${sessionId}.json`),
      JSON.stringify({ gitBranch: 'feat/from-state', activePlan: 'plans/from-state' })
    );
    // Codex has no env-file → CK_* unset
    delete process.env.CK_GIT_BRANCH;
    delete process.env.CK_ACTIVE_PLAN;

    const data = extractSessionData({ session_id: sessionId });
    assert.strictEqual(data.branch, 'feat/from-state', 'branch must come from session state');
    assert.strictEqual(data.plan, 'plans/from-state', 'plan must come from session state');
  });

  it('falls back to CK_* env when session state is absent (Claude parity)', () => {
    const sessionId = `extract-env-fallback-${Date.now()}`; // no state file written
    process.env.CK_GIT_BRANCH = 'feat/from-env';
    process.env.CK_ACTIVE_PLAN = 'plans/from-env';

    const data = extractSessionData({ session_id: sessionId });
    assert.strictEqual(data.branch, 'feat/from-env', 'branch falls back to env');
    assert.strictEqual(data.plan, 'plans/from-env', 'plan falls back to env');
  });
});
