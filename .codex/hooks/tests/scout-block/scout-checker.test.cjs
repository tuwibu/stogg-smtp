 ...
 *
 * Run: node --test ..codex/hooks/tests/scout-block/scout-checker.test.cjs
 */
...
describe('isVenvExecutable', () => {
  ...
  it('matches nested .venv path', () => {
    assert.ok(isVenvExecutable('~/.agents/skills/.venv/bin/python3 script.py'));
  });
...
describe('checkScoutBlock - Bash commands', () => {
  ...
  describe('allowed venv commands', () => {
    it('allows .venv/bin/python3 execution', () => {
      const r = checkScoutBlock({ toolName: 'Bash', toolInput: { command: '~/.agents/skills/.venv/bin/python3 script.py' }, options: DEFAULT_OPTS });
      assert.ok(!r.blocked);
    });
