...
function isHighLevelPath(basePath, cwd) {
  ...
  // A path that targets a known source directory is NOT high-risk, even if shallow.
  // Scoping a broad pattern via the `path` param (e.g. path='src', 'apps/web',
  // 'packages/api') should work — otherwise the only way to scope is to embed the
  // dir in the pattern itself, which is an unintuitive false-positive.
  // This must run BEFORE the shallow-path check below, which would otherwise flag
  // a bare 'src' as high-risk.
  const targetsSpecificDir = SPECIFIC_DIRS.some(dir =>
    normalized === dir ||
    normalized.startsWith(`${dir}/`) ||
    normalized.endsWith(`/${dir}`) ||
    normalized.includes(`/${dir}/`)
  );
  if (targetsSpecificDir) {
    return false;
  }
