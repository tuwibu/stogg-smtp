...
function matchPath(matcher, testPath) {
  ...
  // Strip Windows drive-letter prefix (C:/..., D:/...) — the ignore lib rejects
  // absolute paths, and these never start with '/' so the loop below misses them.
  normalized = normalized.replace(/^[A-Za-z]:\//, '');

  // Strip leading / for absolute paths (ignore lib requires relative paths).
  // Also handles UNC paths (//server/share/...) after the backslash conversion above.
