...
const scenarios = [
  ...
  },
  {
    ...
    expected: 'BLOCKED',
    desc: 'Glob all .ts files (broad pattern at root, blocked)'
  },
  {
  ...
  {
    input: { tool_name: 'Glob', tool_input: { pattern: '**/*.ts', path: 'src' } },
    expected: 'ALLOWED',
    desc: 'Glob scoped via path=src (specific source dir, not high-risk)'
  },
  {
    input: { tool_name: 'Glob', tool_input: { pattern: '**/*.ts', path: 'apps/web' } },
    expected: 'ALLOWED',
    desc: 'Glob scoped via path=apps/web (specific source dir)'
  },
  {
    input: { tool_name: 'Grep', tool_input: { pattern: '*.ts' } },
    expected: 'ALLOWED',
    desc: 'Grep content regex "*.ts" is not a broad glob'
  },
  {
    input: { tool_name: 'Grep', tool_input: { pattern: '\\*\\.json', path: 'src' } },
    expected: 'ALLOWED',
    desc: 'Grep content regex in src is not a broad glob'
  },
