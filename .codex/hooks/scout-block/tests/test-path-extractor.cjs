...
const commandTests = [
  ...
  // Bare blocked dir names (build, dist, ...) are extracted only for filesystem
  // commands (cd, ls, cat, ...). For package-manager commands the bare 'build'
  // token is a subcommand, not a path, so nothing is extracted. Build-tool
  // allowlisting happens at the dispatcher level (isAllowedCommand), not here.
  { cmd: 'npm run build', hasPath: null, desc: 'npm run build (no blocked path)' },
  { cmd: 'pnpm build', hasPath: null, desc: 'pnpm build (no blocked path)' },
...
const looksLikePathTests = [
  ...
  { str: 'node_modules', expected: false, desc: 'bare dir name (not structurally path-like; caught by isBlockedDirName in fs-command context)' },
