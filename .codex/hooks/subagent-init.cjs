 ...
 *
 * Fires: When a subagent (Codex subagent workflow call) is started
...
/**
 * Build claudekit plan CLI reference for plan-aware agents (~50 tokens)
 * Provides deterministic plan status commands instead of manual markdown editing
 * @param {string} agentType
 * @param {string} activePlan - absolute path to active plan dir, or empty string
 */
function buildPlanCliSection(agentType, activePlan) {
  if (!PLAN_AWARE_AGENTS.has(agentType)) return [];
  const dirSuffix = activePlan ? ` --dir "${activePlan}"` : '';
  return [
    ``,
    `## Plan CLI (deterministic updates)`,
    `\`claudekit plan check <id>${dirSuffix}\` = completed | \`claudekit plan check <id> --start${dirSuffix}\` = in-progress | \`claudekit plan uncheck <id>${dirSuffix}\` = revert`,
    `Detect by running \`claudekit version\` & checking exit 0 (works in any shell; NOT \`--version\`, NOT bash-only \`command -v\`). Fallback: if unavailable, edit plan.md Status column directly.`
  ];
}
...
async function main() {
  ...
  try {
    ...
    lines.push(...buildPlanCliSection(agentType, activePlan));
