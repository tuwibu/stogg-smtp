...
function countConfigs(cwd) {
  ...
  const claudeDir = path.join(homeDir, '.codex');

  // User scope
  if (fs.existsSync(path.join(claudeDir, 'AGENTS.md'))) claudeMdCount++;
  ...
  // Project scope
  if (cwd) {
    if (fs.existsSync(path.join(cwd, 'AGENTS.md'))) claudeMdCount++;
    if (fs.existsSync(path.join(cwd, 'CLAUDE.local.md'))) claudeMdCount++;
    if (fs.existsSync(path.join(cwd, 'AGENTS.md'))) claudeMdCount++;
    if (fs.existsSync(path.join(cwd, 'AGENTS.override.md'))) claudeMdCount++;
