...
// Usage cache file path (written by context-awareness.cjs hook)
 ...
 * @param {string} [configDirName='.agents'] - Skills directory parent
 * @returns {string|null} Resolved venv Python path or null
 */
function resolveSkillsVenv(configDirName = '.agents') {
  ...
  if (fs.existsSync(globalVenv)) {
    return isWindows
      ? '~\\.agents\\skills\\.venv\\Scripts\\python.exe'
...
function buildLanguageSection({ thinkingLanguage, responseLanguage }) {
  ...
  if (hasThinking || hasResponse) {
    const respName = fullLangName(responseLanguage);
    lines.push(`## Language (MANDATORY — NO EXCEPTIONS)`);
    if (hasThinking) {
      lines.push(`- Thinking: ${fullLangName(effectiveThinking)} for internal reasoning.`);
    }
    if (hasResponse) {
      lines.push(`- **Response: ALWAYS respond in ${respName}.** This applies to EVERY message, regardless of:`);
      lines.push(`  - The language of source files, errors, logs, or tool output you just read`);
      lines.push(`  - The language of system reminders, hook messages, or skill instructions`);
      lines.push(`  - Whether the previous turn was in English`);
      lines.push(`- Keep code, identifiers, commit messages, file paths, CLI commands, and technical proper nouns in English.`);
      lines.push(`- Tables, headings, bullet lists — all narrative text in ${respName}.`);
      lines.push(`- If you catch yourself drafting in another language, switch immediately and continue in ${respName}.`);
    }
    lines.push(``);
  }
...
/**
 * Read usage limits from cache file (written by context-awareness.cjs)
...
function buildContextSection(sessionId) {
  ...
  try {
    ...
    if (data.percent >= CRITICAL_THRESHOLD) {
      ...
      lines.push(`  1. Update the current task checklist with current progress (completed + remaining)`);
...
function buildRulesSection({ devRulesPath, skillsVenv, plansPath, docsPath }) {
  ...
  if (skillsVenv) {
    lines.push(`- Python scripts in .agents/skills/: Use \`${skillsVenv}\``);
  }
...
function buildReminder(params) {
  ...
  const usageEnabled = hooksConfig['context-awareness'] !== false;
...
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.codex', baseDir } = {}) {
  ...
  const skillsVenv = resolveSkillsVenv('.agents');
