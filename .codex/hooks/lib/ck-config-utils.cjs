...
const FALLBACK_LOCAL_CONFIG_PATH = '.claude/.ck.json';
const LEGACY_LOCAL_CONFIG_PATH = '.claude/.claude-config.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.codex', '.ck.json');
const FALLBACK_GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.ck.json');
...
const DEFAULT_CONFIG = {
  ...
  workflow: {
    ...
    simplifyThreshold: { maxLoc: 30, maxFiles: 2 },
    chains: {
...
function loadFirstConfigFromPaths(configPaths) {
...
function loadConfig(options = {}) {
  ...
  // Load Codex configs first, then Claude configs as compatibility fallback.
  const globalConfig = loadFirstConfigFromPaths([GLOBAL_CONFIG_PATH, FALLBACK_GLOBAL_CONFIG_PATH]);
  const localConfig = loadFirstConfigFromPaths([
    path.join(projectRoot, LOCAL_CONFIG_PATH),
    path.join(gitRoot, LOCAL_CONFIG_PATH),
    path.join(projectRoot, FALLBACK_LOCAL_CONFIG_PATH),
    path.join(gitRoot, FALLBACK_LOCAL_CONFIG_PATH)
  ]);
  const legacyConfig = !localConfig ? loadFirstConfigFromPaths([
    path.join(projectRoot, LEGACY_LOCAL_CONFIG_PATH),
    path.join(gitRoot, LEGACY_LOCAL_CONFIG_PATH)
  ]) : null;

  // No config files found - use defaults
  if (!globalConfig && !localConfig && !legacyConfig) {
    return getDefaultConfig(includeProject, includeAssertions, includeLocale);
  }

  try {
    // Deep merge: DEFAULT → global → legacy → local (local wins)
    ...
    // Workflow chains (consumed by workflow-chaining rule + simplify-eligibility hook)
...
/**
 * Read a per-hook option from config.
 * When config.hooks[hookName] is a plain object (not boolean, null, or array),
 * returns the value at [key] if present; otherwise returns defaultValue.
 *
 * Leaves isHookEnabled behavior unchanged: an object value is !== false, so
 * the hook remains enabled when configured as an options object.
 *
 * @param {string} hookName - Hook name (script basename without .cjs)
 * @param {string} key - Option key to read from the hook's config object
 * @param {*} defaultValue - Value to return when key is absent or hook is not an object
 * @returns {*} The option value or defaultValue
 */
function getHookOption(hookName, key, defaultValue) {
  const config = loadConfig({ includeProject: false, includeAssertions: false, includeLocale: false });
  const hookValue = (config.hooks || {})[hookName];
  // Only read keys when the value is a plain object (not boolean, null, array, etc.)
  if (hookValue !== null && typeof hookValue === 'object' && !Array.isArray(hookValue)) {
    return key in hookValue ? hookValue[key] : defaultValue;
  }
  return defaultValue;
}
...
module.exports = {
  CONFIG_PATH,
  LOCAL_CONFIG_PATH,
  FALLBACK_LOCAL_CONFIG_PATH,
  LEGACY_LOCAL_CONFIG_PATH,
  GLOBAL_CONFIG_PATH,
  FALLBACK_GLOBAL_CONFIG_PATH,
  DEFAULT_CONFIG,
  INVALID_FILENAME_CHARS,
  deepMerge,
  loadConfigFromPath,
  loadFirstConfigFromPaths,
  loadConfig,
  normalizePath,
  isAbsolutePath,
  sanitizePath,
