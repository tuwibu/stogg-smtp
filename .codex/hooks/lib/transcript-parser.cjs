...
function processEntry(entry, toolMap, agentMap, latestTodos, result) {
  ...
  for (const block of content) {
    ...
    if (block.type === 'tool_use' && block.id && block.name) {
      if (block.name === 'Task') {
      ...
      } else if (block.name === 'the current task checklist') {
        result.statuslineActivityCount += 1;
        hadActivity = true;
      ...
      } else if (block.name === 'task tracking') {
        result.statuslineActivityCount += 1;
        hadActivity = true;
        ...
        // Numeric fallback maps to native-task creation order only (not legacy the current task checklist items).
