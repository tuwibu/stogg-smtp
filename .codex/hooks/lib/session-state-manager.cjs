 ...
 * Storage: <projectRoot>/session-state/ — sibling of docs/ and plans/.
 *   Lives outside .claude/ so auto-updates that wipe .claude/ don't drop state.
...
/** Resolve state dir: <projectRoot>/session-state/ — survives .claude/ auto-updates */
function getStateDir(cwd) {
  try {
    if (!cwd) return null;
    const stateDir = path.join(cwd, 'session-state');
...
function extractSessionData(stdinData) {
  const sessionId = stdinData.session_id || process.env.CK_SESSION_ID || '';
  // Codex has no env-file mechanism, so CK_* are never injected there. Source branch/plan
  // from session state (session-init writes both on Codex and Claude); env is the Claude
  // fallback where the env-file path still populates CK_GIT_BRANCH/CK_ACTIVE_PLAN.
  const sessionState = sessionId ? readSessionState(sessionId) : null;
  const data = {
    timestamp: new Date().toISOString(),
    branch: sessionState?.gitBranch || process.env.CK_GIT_BRANCH || '',
    plan: sessionState?.activePlan || process.env.CK_ACTIVE_PLAN || '',
    todos: [], modifiedFiles: []
  };
  const cachedSnapshot = sessionState?.statusline || null;
      ...
      for (const line of lines) {
        try {
          ...
          for (const block of blocks) {
            if (block.type === 'tool_use' && block.name === 'the current task checklist' && Array.isArray(block.input?.todos)) {
              latest.length = 0;
