...
// loadConfig: .codex first, .claude as compatibility fallback
console.log('\n=== loadConfig local config resolution (.codex → .claude fallback) ===\n');

test('loadConfig falls back to .claude/.ck.json when .codex/.ck.json absent', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ckcfg-fallback-'));
  const origCwd = process.cwd();
  try {
    fs.mkdirSync(path.join(tempDir, '.claude'), { recursive: true });
    fs.writeFileSync(path.join(tempDir, '.claude', '.ck.json'), JSON.stringify({ codingLevel: 3 }));
    process.chdir(tempDir);
    const config = loadConfig();
    // Default codingLevel is -1; reading 3 proves the .claude fallback was used.
    assertEquals(config.codingLevel, 3);
  } finally {
...
test('loadConfig prefers .codex/.ck.json over .claude/.ck.json when both exist', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ckcfg-priority-'));
  const origCwd = process.cwd();
  try {
    fs.mkdirSync(path.join(tempDir, '.codex'), { recursive: true });
    fs.mkdirSync(path.join(tempDir, '.claude'), { recursive: true });
    fs.writeFileSync(path.join(tempDir, '.codex', '.ck.json'), JSON.stringify({ codingLevel: 2 }));
    fs.writeFileSync(path.join(tempDir, '.claude', '.ck.json'), JSON.stringify({ codingLevel: 3 }));
    process.chdir(tempDir);
    const config = loadConfig();
    assertEquals(config.codingLevel, 2);
  } finally {
    process.chdir(origCwd);
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
