...
test('processEntry task tracking numeric fallback targets native tasks only', () => {
  const toolMap = new Map();
  const agentMap = new Map();
  const latestTodos = [];
  const result = { sessionStart: null };
...
test('processEntry hydrates native task id from task tracking tool_result', () => {
  const toolMap = new Map();
  const agentMap = new Map();
  const latestTodos = [];
  const result = { sessionStart: null };
  ...
  assertEquals(latestTodos[0].id, 'task-123', 'task tracking result should hydrate native task id');
  assertEquals(latestTodos[0].status, 'in_progress', 'Hydrated id should allow direct task tracking matching');
  assertEquals(latestTodos[0].activeForm, 'Working task one', 'task tracking should refresh activeForm');
});
