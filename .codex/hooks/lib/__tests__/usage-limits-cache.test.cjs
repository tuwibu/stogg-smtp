...
async function main() {
  ...
  await test('runtime override detection hides quota for third-party Codex launches', async () => {
    assertTrue(
      hasAnthropicRuntimeOverride({
  ...
  await test('supported subscription detection accepts native Codex plans and rejects free/non-sub auth', async () => {
    assertTrue(
    ...
    assertTrue(
      ...
      'Rate-limit tier should provide a defensive fallback for native Codex plans'
    );
    assertTrue(
