...
async function main() {
  ...
  await test('Mode compact keeps output to two lines', async () => {
    ...
    try {
      ...
      assertContains(stdout, 'Codex', 'Compact mode should include model');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  await test('Default/full mode includes model and path', async () => {
    ...
    const { stdout } = result;
    assertContains(stdout, 'Codex', 'Full mode should include model');
  ...
  await test('Native task tracking/task tracking flow is rendered', async () => {
    ...
    try {
      ...
      const result = runStatuslineSync({ payload });
      assertSuccessfulRun(result, 'Native task tracking/task tracking scenario');
      const { stdout } = result;
  ...
  await test('Mixed the current task checklist + Native task tracking keeps legacy todo unchanged', async () => {
    ...
    try {
      ...
      const { stdout } = result;
      assertContains(stdout, 'Working native first', 'Native fallback should map to native tasks, not legacy the current task checklist');
      assertTrue(!stdout.includes('Legacy first'), 'Legacy the current task checklist item should not be promoted to active task');
      assertTrue(!stdout.includes('Legacy second'), 'Legacy the current task checklist list should not leak into native active line');
    } finally {
      try { fs.unlinkSync(sessionPath); } catch {}
    }
  });
