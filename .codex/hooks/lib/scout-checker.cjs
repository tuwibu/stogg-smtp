...
// Runtime executors that RUN a compiled/script entrypoint, e.g. `node dist/main.js`,
// `tsx src/main.ts`, `bun build/server.js`. Allowed so the agent can execute build
// output, while file readers (cat, grep, less, ...) stay blocked from dist/build.
// deno/python/ruby/php are already covered by TOOL_COMMAND_PATTERN.
const RUNTIME_EXECUTOR_PATTERN = /^(\.\/)?(node|nodejs|ts-node|tsx|bun)\b/;
...
/**
 * Check if command runs a compiled/script entrypoint via a JS/TS runtime
 * (node, ts-node, tsx, bun). Lets the agent execute build output while
 * file readers (cat/grep/less) remain blocked from dist/build.
 * @param {string} command - The command to check
 * @returns {boolean}
 */
function isRuntimeExecutor(command) {
  if (!command || typeof command !== 'string') return false;
  return RUNTIME_EXECUTOR_PATTERN.test(command.trim());
}

/**
 * Check if command should be allowed (build, venv executable, venv creation,
 * or a JS/TS runtime executing a file).
...
function isAllowedCommand(command) {
  ...
  return isBuildCommand(stripped)
    || isVenvExecutable(stripped)
    || isVenvCreationCommand(stripped)
    || isRuntimeExecutor(stripped);
}
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  ...
  // Check for overly broad glob patterns — ONLY for the Glob tool.
  // Other tools (notably Grep) use `pattern` as a content regex, not a glob,
  // so running glob-breadth detection on them blocks legitimate searches
  // (e.g. Grep pattern "*.ts" is a literal search, not a broad glob).
  if (checkBroadPatterns && toolName === 'Glob') {
