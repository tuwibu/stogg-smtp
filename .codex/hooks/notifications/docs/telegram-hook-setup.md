...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "node ${CLAUDE_PROJECT_DIR}/..codex/hooks/notifications/notify.cjs"
      }]
    }],
    "SubagentStop": [{
      "hooks": [{
...
**Cause:** Codex hook configuration incorrect
...
**Implementation:** `..codex/hooks/notifications/notify.cjs` + `providers/telegram.cjs`
