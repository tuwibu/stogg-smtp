#!/usr/bin/env python3
"""
Scan .agents/skills and regenerate the checked-in skill registries.
...
def write_catalog_yaml(skills: list[dict], repo_root: Path) -> Path:
    ...
    lines = [
        ...
        f"  description: {_qs('Auto-generated catalog of all available skills in ClaudeKit for Codex')}",
...
def write_catalog_markdown(skills: list[dict], repo_root: Path) -> Path:
    ...
    lines = [
        "# Skills Catalog", "",
        "Auto-generated catalog of all available skills in ClaudeKit for Codex.", "",
    ...
    for cat in active:
        ...
        for skill in sorted(cats[cat], key=lambda s: str(s["display_name"]).lower()):
            ...
            lines.extend([
                ...
                desc_safe, "",
                f"**Location**: `.agents/skills/{skill['path']}`", "",
            ])
    out.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
    return out
