...
- Use `debug` skill for analyzing code, debugging, etc. if needed
...
- Use `code-reviewer` agent to review code when the change is high-risk (auth/secrets/payments/DB schema/migration/public API/CI/deploy/destructive FS) or large-diff, or when the user requests it — not mandatory for small low-risk changes
