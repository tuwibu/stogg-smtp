...
> For chain transitions (which skill to suggest after current completes), see [`workflow-chaining.md`](./workflow-chaining.md). For reuse-first audit before any new code, see [`reuse-first.md`](./reuse-first.md). For context-window discipline (compaction thresholds), see [`context-hygiene.md`](./context-hygiene.md).
...
#### 3. Code Quality
- After testing passes, delegate to `code-reviewer` agent for high-risk or large-diff changes (or when the user requests it); skip for small low-risk changes.
...
- **Markdown mode:** Auto-opens in browser with Mermaid rendering
