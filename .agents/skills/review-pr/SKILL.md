...
Before each `/git cp`, re-check that `git rev-parse --abbrev-ref HEAD` and `git rev-parse HEAD` still match the PR head branch/SHA or the expected local fix commit descendant.

### 3. Fix code and config blockers

Activate `/fix --auto` with the full blocking set and PR context:

```
/fix --auto "Fix all blockers from review-pr <PR_REF>: <finding, conflict, and CI summary>"
```
...
`/fix` performs its own scout, diagnose, implementation, verification, and prevention flow. Do not bypass its hard gates. If a blocker is external state such as missing credentials, service outage, unavailable approval, or a secret value not present anywhere accessible, record it as an external blocker instead of weakening tests or hiding the failure.
...
After fixes verify locally, commit and push the fixes to the PR head branch. Do not push if verification failed, secrets are detected, or the working tree contains unrelated user changes.

Then watch all PR checks:
- Start with `gh pr checks "$PR_REF"`.
- If any check is pending, run `gh run watch <run-id> --timeout 1800` (30-minute hard ceiling) and wait for terminal state. **Do NOT poll in a loop without a timeout — if the command exits non-zero or times out, classify as an external CI blocker and STOP the loop.**
- If any check fails after watching, inspect logs, add the failure to the blocking set, and loop back to Step 1.
- If rerunning is appropriate for a clearly transient infrastructure failure, use `gh run rerun <run-id> --failed`, then watch the rerun with `gh run watch <run-id> --timeout 1800`. Do not rerun to hide deterministic failures.
- If required checks remain queued, pending, or skipped after the 1800-second timeout without new log evidence, classify them as external CI blockers with run/job links and **STOP** — do not retry indefinitely.
...
```
*Posted by /review-pr at <ISO-8601 UTC timestamp>*
```
...
If the loop terminates due to a blocker (non-converging, `/fix` blocked, CI unresolvable), still post the final review — but the verdict will reflect remaining findings (likely **Request changes** or **Comment**), and the body should include the blocker so the human reviewer knows where to take over.
...
## Auto-merge guard (`gh pr merge --auto`)

Before calling `gh pr merge --auto` for any PR:

1. Print PR title, number, and target branch clearly.
2. Confirm with the user via `AskUserQuestion` — this is a one-way action.
3. **Abort if target = default branch AND the PR has not yet received a human approval** (`gh pr view "$PR_REF" --json reviewDecision` returns `APPROVED` from a reviewer who is not the PR author). Do not proceed if `reviewDecision` is `REVIEW_REQUIRED` or `null`.

This guard applies regardless of CI state — green CI alone does not substitute for human code review on the default branch.
