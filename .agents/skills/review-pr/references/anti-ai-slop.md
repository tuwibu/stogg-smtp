...
> **Alignment note**: This taxonomy aligns with the AI-assisted code risk lens in `.codex/agents/code-reviewer.md`. The same patterns (parallel reimplementation, phantom tests, scope drift, abstraction without domain anchor, polished-but-empty comments) are flagged there and here — the difference is scope: `code-reviewer` is for local codebase review, this reference is loaded during PR review when the full slop taxonomy is needed.
