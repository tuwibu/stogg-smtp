...
**Progress tracking:** Use the `update_plan` tool to track workflow tasks. If Codex subagent workflows are unavailable (VSCode extension), keep progress in an `update_plan` checklist. All workflow steps remain functional either way.
...
4. Use `update_plan` to create workflow step tasks (with dependencies if complex)
...
**IMPORTANT:**
1. `update_plan` first — check for existing tasks (hydrated by planning skill in same session)
2. If tasks exist → pick them up, skip re-creation
3. If no tasks → read plan phases, `update_plan` for each unchecked `[ ]` item with priority order and metadata (`phase`, `planDir`, `phaseFile`)
...
**All modes:**
- Use `update_plan` to mark tasks as `in_progress` immediately.
...
**Parallel mode:**
- Use the `update_plan` tool to track tasks across the parallel run
- Launch multiple `developer` agents
- When agents pick up a task, use `update_plan` to assign task to agent and mark tasks as `in_progress` immediately.
...
`.codex/skills/_shared/references/workflow-artifacts.md` and run:

```bash
node .codex/hooks/workflow-artifact-gate.cjs --stage finalize --artifact-dir <artifact-dir>
```
...
1. **MUST** activate `/project-management` skill (MANDATORY) — run full sync-back for [plan-path]: reconcile all completed Codex tasks with all phase files, backfill stale completed checkboxes across every phase, then update plan.md frontmatter/table progress. Do NOT only mark current phase.
...
4. Use `update_plan` to mark Codex tasks complete after sync-back confirmation.
...
- Use `update_plan` to track each unchecked item with priority order and dependencies.
- Use `update_plan` to mark a task `in_progress` when picking it up (skip if Codex subagent workflows unavailable).
- Use `update_plan` to mark a task `complete` immediately after finalizing it (skip if Codex subagent workflows unavailable).
