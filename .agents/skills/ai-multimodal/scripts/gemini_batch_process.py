...
def find_api_key() -> Optional[str]:
    ...
    5. ~/.agents/skills/ai-multimodal/.env (user skill-specific)
    6. ~/.agents/skills/.env (user shared)
...
def batch_process(
    ...
    if requires_google_sdk:
        ...
        if not api_key:
            ...
            print("  echo 'GEMINI_API_KEY=your-key' >> ~/.codex/.env")
            print("\nOther options:")
            print("  - Run setup checker: python scripts/check_setup.py")
            print("  - Show full hierarchy: python ~/.codex/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal -v")
