...
> **Note:** `init` no longer generates per-domain `docs/flows/<domain>-flow.md` (deprecated — it
> duplicated `docs/services/<svc>/flows.md`). The `docs/flows/` directory now holds cross-service
> **business-process** docs, owned exclusively by `/docs flows` (see `business-flows-workflow.md`).
> The brainstorm/plan `--preview` (inline diagram in `plan.md`) is a separate mechanism, unaffected.
