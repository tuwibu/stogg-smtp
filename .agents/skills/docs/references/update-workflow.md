...
## Additional requests
<!-- $ARGUMENTS substituted here excludes the --preview token (deprecated for update — see note below) -->
<additional_requests>
  $ARGUMENTS
</additional_requests>

> **Note:** `update` no longer regenerates per-domain `docs/flows/<domain>-flow.md` (deprecated — it
> duplicated `docs/services/<svc>/flows.md`). Cross-service **business-process** docs under
> `docs/flows/` are refreshed via `/docs flows --update` (see `business-flows-workflow.md`). The
> brainstorm/plan `--preview` (inline diagram in `plan.md`) is a separate mechanism, unaffected.

## Phase 3: Size Check (Post-Update)

After docs-manager completes:
1. Run `wc -l docs/*.md docs/flows/*.md 2>/dev/null | sort -rn` to check LOC
