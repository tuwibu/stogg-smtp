...
if (require.main === module) {
  ...
  if (args.length < 1) {
    console.log('Usage: node polar-webhook-verify.js <webhook-payload-json>');
    console.log('\nWebhook secret MUST be provided via POLAR_WEBHOOK_SECRET environment variable');
    console.log('\nExample:');
    console.log('  POLAR_WEBHOOK_SECRET=base64secret node polar-webhook-verify.js \'{"type":"order.paid","data":{...}}\'');
    process.exit(1);
  }

  try {
    const payload = args[0];
    // Secret read from env only — never from argv to avoid leaking in process list
