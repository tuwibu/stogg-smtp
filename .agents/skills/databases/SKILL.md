...
## READ-ONLY POLICY (MANDATORY)

When querying any live database (`psql`, `mongosh`, `mysql`, `sqlite3`, `redis-cli`):

- **ALLOWED**: `SELECT`, `EXPLAIN`, `ANALYZE`, `SHOW`, `DESCRIBE`, `\d`, `find()`, `aggregate()`, `count()`, `GET`, `KEYS`, `INFO`
- **FORBIDDEN**: `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`, `DROP`, `ALTER`, `CREATE`, `GRANT`, `REVOKE`, `FLUSHALL`, `FLUSHDB`, `db.X.drop()`, `insertOne/Many`, `update*`, `delete*`, `replaceOne`

If a destructive op is needed:
1. Write the SQL/script to a file (`./tmp/migration-XXX.sql`)
2. Show the user the file + ask them to run it manually
3. Never pipe destructive SQL into a DB client yourself

The `db-readonly-guard` hook enforces this at the Bash layer — destructive commands will be blocked.
