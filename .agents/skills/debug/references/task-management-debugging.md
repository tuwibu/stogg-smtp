...
Track investigation and debugging pipelines via Codex task plan (update_plan, update_plan, update_plan).
...
```
update_plan: "Assess incident scope"      → pending
update_plan: "Collect logs and evidence"  → pending, blockedBy: [assess]
update_plan: "Analyze root cause"         → pending, blockedBy: [collect]
update_plan: "Implement fix"              → pending, blockedBy: [analyze]
update_plan: "Verify fix resolves issue"  → pending, blockedBy: [fix]
```
...
```
update_plan(subject: "Re-analyze: fix attempt {N} failed",
...
If `update_plan` fails: log warning, continue with sequential debugging. Tasks add visibility and coordination, not core functionality.
