...
Track investigation pipelines via Codex task plan (update_plan, update_plan, update_plan). Hydration pattern for multi-step investigations with dependency chains and parallel evidence collection. **Fallback:** Codex subagent workflows are CLI-only — if unavailable (VSCode extension), use `update_plan` checklist for tracking. Debug workflow remains fully functional.
...
Visual verification of frontend implementations via `browser`, `browser`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests. Detect if frontend-related → check browser tool availability → screenshot + console error check → report. Skip if not frontend.
...
- **Frontend:** `browser`, `browser`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests for visual verification
