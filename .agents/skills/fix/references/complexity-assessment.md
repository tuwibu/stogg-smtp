...
### Parallel (→ multiple developer agents) — Use Task Trees
...
**Task usage:** Create separate task trees per independent issue (each with scout+diagnose+fix+verify). Spawn `developer` agent per tree. See `references/task-orchestration.md`.
