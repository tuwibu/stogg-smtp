...
**Task Orchestration (Moderate+ only):** After classifying, create native Codex task plan items for all phases upfront with dependencies. See `references/task-orchestration.md`.
- Skip for Quick workflow (< 3 steps, overhead exceeds benefit)
- Use `update_plan` with `addBlockedBy` for dependency chains
- Update via `update_plan` as each phase completes
- For Parallel: create separate task trees per independent issue
- **Fallback:** Codex subagent workflows (`update_plan`/`update_plan`/`update_plan`/`update_plan`) are CLI-only — unavailable in VSCode extension. If they error, use `update_plan` checklist for progress tracking. Fix workflow remains fully functional without them.
...
2. **Activate `/project-management` skill (MANDATORY)** → sync plan/task status (if fix is part of a plan), update progress, hydrate Codex task plan items, generate status report
3. `docs-manager` subagent → update `./docs` if changes warrant (NON-OPTIONAL)
4. `update_plan` → mark ALL Codex task plan items `completed` (skip if Codex subagent workflows unavailable)
