...
Codex tasks are **ephemeral** — they die when the session ends. `~/.codex/tasks/` holds lock files only, NOT task data. Plan files (plan.md, phase-XX.md with checkboxes) are the **persistent** layer.

**Tool Availability:** The `update_plan` tool tracks tasks in-session. If Codex subagent workflows are unavailable (VSCode extension), keep an `update_plan` checklist. Plan files remain the source of truth; hydration is an optimization, not a requirement.
...
- **Hydrate:** Read plan files → update_plan per unchecked `[ ]` item
- **Work:** update_plan tracks in_progress/completed in real-time
...
3. If the user approves implementation, Cook Step 3: `update_plan` → finds existing tasks → picks them up
