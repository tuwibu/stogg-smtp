...
**Guard:** Skip to Step 3 **only if** `plan.md` contains a `### Verification Results` block under
`## Red Team Review` reporting `Failed: 0` — then limit this step to resolving any remaining
`[UNVERIFIED]` tags. Otherwise run the pass in full.

A `## Red Team Review` heading on its own is **not** sufficient. An unverified red-team is not a
verification, and skipping on the bare heading is how both sides end up assuming the other checked.
...
## Next Steps
**Pre-cook diagram refresh:** if `plan.md` frontmatter has `preview: true`, (re)generate the inline mermaid business-flow diagram in `plan.md` from the validated requirements BEFORE offering `/cook` (see `.claude/rules/business-flow-diagram.md`).
