...
Each reviewer prompt MUST include override, READ-ONLY constraint, persona, plan file paths, and hostile instructions.
Load: `references/red-team-personas.md` for reviewer prompt template.

**READ-ONLY (mandatory):** `code-reviewer` carries Write/Edit. Every reviewer prompt MUST forbid
writing anything but its own report — never `plan.md`, never a `phase-*.md`. Only the controller
edits plan files, and only in Step 8, after the user adjudicates in Step 7. A reviewer that edits a
phase file bypasses the approval gate entirely: the user then approves a plan that was already
...
Evidence comes in two forms. A finding passes the filter if its `Evidence:` field contains **either**:

- **Positive evidence** — one or more `file:line` citations (pattern `path/to/file.ext:NNN`).
- **Negative evidence** — the exact search that was run and came back empty, written so the
  adjudicator can re-run it verbatim: `grep -rn "rateLimit" src/ → 0 matches`.

Negative evidence exists because the most dangerous findings have no line to cite: *"there is no rate
limiting anywhere"*, *"phase 3 has no rollback path"*. The defect **is** the missing line. A filter
that demands a `file:line` for those throws away exactly the class of finding red-team exists to catch.

A finding with neither form → auto-**Reject**, rationale "No codebase evidence." Do not evaluate its
merit.

### Step 6: Adjudicate

**Verify before merit.** A citation nobody opens is a citation a model can invent.

1. **Positive citations:** open the file at that line. If the file does not exist, or the line does not
   support the claim → **Reject** ("citation not verified"), and mark the emitting reviewer
   **unreliable**.
2. **Negative citations:** re-run the stated command. Non-empty result → **Reject** ("negative evidence
   does not reproduce").
3. A reviewer marked **unreliable** does not have its remaining findings auto-rejected — a fabricated
   citation on one finding does not make the others false, only untrusted. Re-check each of its
   surviving findings against the codebase before judging merit.
4. Only findings that survive verification are evaluated on merit: **Accept** or **Reject**.

Reuse the per-tier claim budgets in `references/verification-roles.md` — do not invent a new budget.
...
**Emit `### Verification Results` (mandatory).** Inside `## Red Team Review`, write the verification
block using the format in `references/verification-roles.md` → "Verification Output Format": claims
checked, verified / failed / unverified counts, and the failure list with `file:line`. This block is
what `/plan validate` reads to decide whether it can skip its own verification pass. Without it,
validate has no way to tell a verified red-team from an unverified one — and will run the pass itself.
...
## Next Steps
**Pre-cook diagram refresh:** if `plan.md` frontmatter has `preview: true`, (re)generate the inline mermaid business-flow diagram in `plan.md` from the post-red-team requirements BEFORE offering `/cook` (see `.codex/rules/business-flow-diagram.md`).
