...
- **Business-flow diagram (`--preview` only):** per `.codex/rules/business-flow-diagram.md` — embed an inline mermaid flow ONLY when invoked with the `--preview` flag; otherwise draw nothing.
...
auto-detection. Rules: `.codex/rules/business-flow-diagram.md`.
