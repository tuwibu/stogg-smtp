...
```
pnpm -C packages/mcp dev          # stdio, for Codex integration
pnpm -C packages/mcp dev -- --transport http --port 8080
```

Add an `mcp.json` example in `docs/mcp.md` showing how to register the server in Codex, Cursor, and similar clients.
