...
- [ ] Env vars: respects hierarchy `process.env` > `$HOME/.agents/skills/${SKILL}/.env` (global) > `$HOME/.agents/skills/.env` (global) > `$HOME/.codex/.env` (global) > `./.agents/skills/${SKILL}/.env` (cwd) > `./.agents/skills/.env` (cwd) > `./.codex/.env` (cwd)
...
When a skill requires subagent delegation (via Codex subagent workflow):
...
3. **Add validation rule** - "If Codex subagent spawns = 0 at end, workflow is INCOMPLETE"
