...
Browser-based visual testing via `browser`, `web-testing`, `browser`, or project-native Playwright/Vitest/k6 commands. Covers screenshots, responsive checks, accessibility audits, form automation, and console error collection.
...
UI tests       → ui-testing-workflow.md
  ...
  Auth: browser for real user login/cookies, or project-native test setup
...
6. For frontend: run UI tests via `browser`, `web-testing`, `browser`, or project-native browser tests
...
- **Browser**: `browser` for live browser testing without real user cookies; `browser` for the user's actual Chrome login state; `web-testing` or project-native Playwright/Vitest/k6 for repeatable UI tests
...
1. On start: check `update_plan`, claim assigned/next unblocked task via `update_plan`
2. Read full task description via `update_plan` before starting
...
5. When done: `update_plan(status: "completed")` then `SendMessage` results to lead

**Fallback:** Codex subagent workflows (`update_plan`/`update_plan`/`update_plan`) are CLI-only — unavailable in VSCode extension. If they error, use `update_plan` checklist for progress tracking and coordinate via `SendMessage` only.
