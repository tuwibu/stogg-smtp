...
**Fallback:** Codex subagent workflows (`update_plan`/`update_plan`/`update_plan`/`update_plan`) are CLI-only — unavailable in VSCode extension. If they error, use `update_plan` checklist for tracking and run pipeline sequentially. Review quality is identical.
