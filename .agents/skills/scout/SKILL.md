...
- **Skip if:** Codex subagent workflows unavailable (VSCode extension) — use `update_plan` checklist instead
- `update_plan` first — check for existing scout tasks in session
- If not found, `update_plan` per agent with scope metadata
...
**Notes:**
- `update_plan` each task to `in_progress` before spawning its agent (skip if Codex subagent workflows unavailable)
...
- `update_plan` completed tasks; log timed-out agents in report (skip if Codex subagent workflows unavailable)
