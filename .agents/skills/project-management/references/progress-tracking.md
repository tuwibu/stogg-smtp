...
Use `claudekit plan` CLI commands for deterministic, format-safe status changes:
...
claudekit plan add-phase "Phase Name" [--after <id>]
```
...
**Fallback:** If `claudekit` CLI is not available, edit `plan.md` directly —
...
Codex task plan items (session-scoped): `pending` → `in_progress` → `completed`
