...
```bash
# NOTE: /review-pr resolves after Phase 5 of skills port — link is intentional placeholder.
/review-pr <PR number or URL> --fix --reply
```
...
## Step 4: Confirm Guard (REQUIRED before merge)

Before each merge, display PR details and require explicit confirmation:

```bash
PR_INFO=$(gh pr view "$PR_REF" --json number,title,baseRefName,mergeStateStatus)
...
- question: "Merge PR #<number> into <baseRefName>?"
- header: "Confirm Merge"
- options: ["Yes, merge", "Abort"]

**If user selects Abort**: stop immediately, do not proceed.

**If target branch = default branch AND merge state is not MERGEABLE or checks are not green**:
```
ABORT: PR #<number> targets default branch (<name>) but is not fully approved/green.
Merge blocked.
```

Never proceed past this guard without explicit user confirmation.
...
3. If the failure is deterministic and repo-fixable, activate:
   ```bash
   /fix --auto "Fix post-merge CI failure from git merge-pr: <workflow, run id, job id, command, exact error>"
   ```
...
confirmed: PR #123 merge approved by user
...
If aborted at confirm guard:

```text
aborted: PR #123 — user declined merge confirmation
```
