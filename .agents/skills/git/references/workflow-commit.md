...
## Tool 0: Identity Guard (MANDATORY — before staging)

Refuse to commit as a Claude/bot identity; restore the repo's real author from history.
```bash
...
if printf '%s %s' "$n" "$e" | grep -qiE 'claude|anthropic|\[bot\]'; then
  ...
  [ -n "$real" ] && git config --local user.name "${real%%|*}" \
    && git config --local user.email "${real##*|}" \
    || { echo "IDENTITY_UNKNOWN — STOP, ask user"; exit 1; }
fi
```
**NEVER** use `--author` / `GIT_AUTHOR_*` / `GIT_COMMITTER_*` to force a Claude author.
...
git diff --cached --name-only | awk -F'/' '{
  ...
  else if ($0 ~ /\.codex/) print "config:"$0
