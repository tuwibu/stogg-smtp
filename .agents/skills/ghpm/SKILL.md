...
- Use least-privilege tokens. Read-only Project operations require `read:project` scope; Project mutations (field updates, item moves) require `project` scope. Verify with `gh auth status` before any Project write.
...
| --- | --- |
| Read Issues, PRs, Actions, labels | `repo` (default) |
| Read GitHub Projects fields/items | `read:project` |
| Write/mutate GitHub Projects (fields, status, item moves) | `project` |

Always start with the minimum scope. Elevate to `project` only when a mutation is needed:

```bash
# Check current scopes
gh auth status

# Add read:project scope (read-only Project access)
gh auth refresh -s read:project

# Add project scope only when writing to Projects
...
## Skill Chain
Suggested skills and order, e.g. /scout -> /plan -> /cook -> /test -> /git -> /ship.
```
