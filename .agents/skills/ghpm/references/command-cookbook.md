...
`gh project` read operations require token scope `read:project`.
`gh project` write/mutate operations require token scope `project`.

```bash
# Verify current token scopes before any Project operation
gh auth status

# Add read:project scope (minimum for browsing Projects)
gh auth refresh -s read:project

# Elevate to project scope only when writing to Projects
...
Use GraphQL for precise field writes (requires `project` scope):
