...
**Date format:** Use `$CK_PLAN_DATE_FORMAT` env var. On Codex (no env-file), read
`plan.dateFormat` from `.codex/.ck.json`. Fallback default `YYMMDD-HHmm`.
...
- `designer` / `frontend-design` → assets/designs/
