...
Use `$CK_PLAN_DATE_FORMAT` env var if set. On Codex this var is not injected (no env-file
mechanism) — read `plan.dateFormat` from `.codex/.ck.json` (or `.claude/.ck.json`) instead.
If neither is available, default to `YYMMDD-HHmm`.
